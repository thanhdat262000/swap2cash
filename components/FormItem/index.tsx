  ...
  return (
    <>
      {label && showLevelPassword && (
        <div className={cx('mb-[2px] form-item__label', labelClassName)}>
      ...
      <Password visibilityToggle={true} {...field} {...props} />
    </>
  );
};
    ...
    <div className={className}>
      ...
      >
        {options.map((item: any) => (
          <Option value={item.value} key={item?.key || item.value} label={item.name} textFilter={item.textFilter}>
            {optionsRender(item)}
          </Option>
        ))}
      </Select>
...
const FormItem = ({
  ...
  return (
    <div className={cx(containerClassName, 'form-item')}>
      {label && (
        <div className={cx(labelClassName, 'mb-[2px] form-item__label')}>
